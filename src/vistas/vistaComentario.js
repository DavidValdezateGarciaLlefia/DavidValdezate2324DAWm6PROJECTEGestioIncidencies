...
export const vistaComentario = {
  ...
  añadirComentario: () => {
    ...
    let comentarioObjeto = {
        ...
        fecha: fechaInput.value.split('T')[0]
    }; 
    objetosBD.comentariosBD.push(comentarioObjeto);
  ...
  insertaComentarios: () => {
    let comentarioHTML = '';
    const comentariosFiltrados = objetosBD.comentariosBD.filter(comentario => comentario.codigo === panel.codigo);
    
    comentariosFiltrados.forEach(comentario => {
        const objeto = objetosBD.objetosBD.find(objeto => objeto.codigo === comentario.codigo);
        if (objeto) {
